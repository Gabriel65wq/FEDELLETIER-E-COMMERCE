...
const CheckoutManager = {
  ...
  setupEventListeners() {
    ...
    const volverCarritoTopBtn = document.getElementById("volver-carrito-top-btn")
    ...
    const datosDomicilioSection = document.getElementById("datos-domicilio-section")
    ...
    if (continuarPagoBtn) {
      continuarPagoBtn.addEventListener("click", () => {
        ...
        this.renderCheckout()
        if (this.checkoutAnimationId) {
          this.animateCheckoutCanvas()
        }
      })
    }

    if (volverCarritoTopBtn) {
      volverCarritoTopBtn.addEventListener("click", () => {
        checkoutSeccion.style.display = "none"
        carritoSeccion.style.display = "flex"
        document.body.style.overflow = "hidden"
        if (this.checkoutAnimationId) {
          cancelAnimationFrame(this.checkoutAnimationId)
        }
      })
    }
    ...
    metodoEntregaRadios.forEach((radio) => {
      radio.addEventListener("change", (e) => {
        if (e.target.value === "retiro") {
          ...
          datosDomicilioSection.style.display = "none"
          document.getElementById("dia-retiro").required = true
          document.getElementById("horario-retiro").required = true
          ...
          document.getElementById("codigo-postal").required = false
        } else {
          coordinarRetiroSection.style.display = "none"
          datosDomicilioSection.style.display = "block"
          document.getElementById("dia-retiro").required = false
          document.getElementById("horario-retiro").required = false
          ...
          document.getElementById("codigo-postal").required = true
        }
      })
    })
    ...
    if (checkoutForm) {
      checkoutForm.addEventListener("submit", (e) => {
        ...
        const horarioRetiro = document.getElementById("horario-retiro").value
        const metodoEntrega = document.querySelector('input[name="metodo-entrega"]:checked').value

        if (metodoEntrega === "retiro" && horarioRetiro) {
          const [hora, minuto] = horarioRetiro.split(":").map(Number)
          if (hora < 9 || hora >= 17) {
            alert("⚠️ El horario de retiro debe estar entre las 9:00 AM y las 5:00 PM")
            return
          }
        }
        ...
        if (metodoEntrega === "retiro") {
          formData.diaRetiro = document.getElementById("dia-retiro").value
          formData.horarioRetiro = horarioRetiro
        } else {
          formData.direccion = document.getElementById("direccion").value
          formData.altura = document.getElementById("altura").value
          formData.pisoDepto = document.getElementById("piso-depto").value
          formData.localidad = document.getElementById("localidad").value
          formData.provincia = document.getElementById("provincia").value
          formData.codigoPostal = document.getElementById("codigo-postal").value
          formData.instruccionesEntrega = document.getElementById("instrucciones-entrega").value
        }

        // Guardar datos en sessionStorage para la factura
        sessionStorage.setItem("checkoutData", JSON.stringify(formData))

        // Mostrar factura
        FacturaManager.mostrarFactura(formData)
  ...
  renderCheckout() {
    ...
    const checkoutCantidadTotal = document.getElementById("checkout-cantidad-total")
    ...
    checkoutCantidadTotal.textContent = CartManager.getTotalItems()
  },

  initCheckoutCanvas() {
    this.checkoutCanvas = document.getElementById("checkout-canvas")
    if (!this.checkoutCanvas) return

    this.checkoutCtx = this.checkoutCanvas.getContext("2d")
    this.checkoutParticles = []
    this.checkoutAnimationId = null

    this.resizeCheckoutCanvas()
    this.createCheckoutParticles()
  ...
  resizeCheckoutCanvas() {
    if (!this.checkoutCanvas) return
    this.checkoutCanvas.width = window.innerWidth
    this.checkoutCanvas.height = window.innerHeight
  },

  createCheckoutParticles() {
    this.checkoutParticles = []
    const particleCount = Math.floor((this.checkoutCanvas.width * this.checkoutCanvas.height) / 20000)

    for (let i = 0; i < particleCount; i++) {
      this.checkoutParticles.push({
        x: Math.random() * this.checkoutCanvas.width,
        y: Math.random() * this.checkoutCanvas.height,
  ...
  animateCheckoutCanvas() {
    if (!this.checkoutCtx) return

    const isDark = document.body.classList.contains("dark")
    this.checkoutCtx.clearRect(0, 0, this.checkoutCanvas.width, this.checkoutCanvas.height)

    this.checkoutParticles.forEach((particle, index) => {
      particle.x += particle.speedX
      particle.y += particle.speedY

      if (particle.x < 0 || particle.x > this.checkoutCanvas.width) {
        particle.speedX *= -1
      }
      if (particle.y < 0 || particle.y > this.checkoutCanvas.height) {
        particle.speedY *= -1
      }
      ...
      const baseColor = isDark
        ? `rgba(0, 212, 255, ${particle.opacity * pulseFactor})`
        : `rgba(0, 168, 255, ${particle.opacity * pulseFactor})`

      this.checkoutCtx.beginPath()
      this.checkoutCtx.arc(particle.x, particle.y, particle.size * pulseFactor, 0, Math.PI * 2)
      this.checkoutCtx.fillStyle = baseColor
      this.checkoutCtx.fill()

      for (let j = index + 1; j < this.checkoutParticles.length; j++) {
        const other = this.checkoutParticles[j]
        const dx = particle.x - other.x
        const dy = particle.y - other.y
        const distance = Math.sqrt(dx * dx + dy * dy)

        if (distance < 100) {
          const lineOpacity = (1 - distance / 100) * 0.1 * pulseFactor
          const lineColor = isDark ? `rgba(0, 212, 255, ${lineOpacity})` : `rgba(0, 168, 255, ${lineOpacity})`

          this.checkoutCtx.beginPath()
          this.checkoutCtx.strokeStyle = lineColor
          this.checkoutCtx.lineWidth = 0.5
          this.checkoutCtx.moveTo(particle.x, particle.y)
          this.checkoutCtx.lineTo(other.x, other.y)
          this.checkoutCtx.stroke()
        }
      }
    })

    this.checkoutAnimationId = requestAnimationFrame(() => this.animateCheckoutCanvas())
  },
}
...
const FacturaManager = {
  init() {
    this.setupEventListeners()
    this.initFacturaCanvas()
  },

  setupEventListeners() {
    const volverCheckoutBtn = document.getElementById("volver-checkout-btn")
    const realizarPedidoBtn = document.getElementById("realizar-pedido-btn")
    const pagarMercadoPagoBtn = document.getElementById("pagar-mercadopago-btn")
    const metodoPagoRadios = document.querySelectorAll('input[name="metodo-pago"]')
    const efectivoDisclaimer = document.getElementById("efectivo-disclaimer")
    const mercadopagoDisclaimer = document.getElementById("mercadopago-disclaimer")
    const efectivoCard = document.getElementById("efectivo-card")
      ...
      metodoPagoRadios.forEach((radio) => {
        radio.addEventListener("change", (e) => {
          if (e.target.value === "efectivo") {
            realizarPedidoBtn.style.display = "flex"
            pagarMercadoPagoBtn.style.display = "none"
            efectivoDisclaimer.style.display = "block"
            mercadopagoDisclaimer.style.display = "none"
          } else {
            realizarPedidoBtn.style.display = "none"
            pagarMercadoPagoBtn.style.display = "flex"
            efectivoDisclaimer.style.display = "none"
            mercadopagoDisclaimer.style.display = "block"
          }
        })
      })
    }
    ...
    // Volver al checkout
    if (volverCheckoutBtn) {
      volverCheckoutBtn.addEventListener("click", () => {
        document.getElementById("factura-seccion").style.display = "none"
        document.getElementById("checkout-seccion").style.display = "block"
        if (this.facturaAnimationId) {
          cancelAnimationFrame(this.facturaAnimationId)
        }
        CheckoutManager.animateCheckoutCanvas()
      })
    }

    // Realizar pedido con efectivo
    if (realizarPedidoBtn) {
      realizarPedidoBtn.addEventListener("click", async () => {
        const checkoutData = JSON.parse(sessionStorage.getItem("checkoutData"))

        realizarPedidoBtn.disabled = true
        realizarPedidoBtn.innerHTML = '<span class="btn-text">Procesando...</span>'
        ...
        try {
          ...
          if (response.ok) {
            ...
            alert(
              "✅ ¡Pedido realizado con éxito!\n\n📞 Federico se pondrá en contacto contigo para confirmar los detalles.\n\n¡Gracias por tu compra!",
            )
          ...
          realizarPedidoBtn.innerHTML =
            '<span class="btn-text">Realizar Pedido</span><svg class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>'
        }
      })
    }
    ...
    if (pagarMercadoPagoBtn) {
      pagarMercadoPagoBtn.addEventListener("click", async () => {
        const checkoutData = JSON.parse(sessionStorage.getItem("checkoutData"))

        pagarMercadoPagoBtn.disabled = true
        pagarMercadoPagoBtn.innerHTML = '<span class="btn-text">Generando link de pago...</span>'

        try {
          const response = await fetch("/api/crear-preferencia-mp", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
          ...
          pagarMercadoPagoBtn.disabled = false
          pagarMercadoPagoBtn.innerHTML =
            '<span class="btn-text">Pagar con MercadoPago</span><svg class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg>'
        }
      })
    }
  },

  mostrarFactura(formData) {
    const facturaSeccion = document.getElementById("factura-seccion")
    const checkoutSeccion = document.getElementById("checkout-seccion")
    const efectivoCard = document.getElementById("efectivo-card")

    // Ocultar checkout y mostrar factura
    checkoutSeccion.style.display = "none"
    facturaSeccion.style.display = "block"

    // Renderizar items
    const facturaItemsLista = document.getElementById("factura-items-lista")
    facturaItemsLista.innerHTML = CartManager.items
      .map(
        (item) => `
      <div class="factura-item">
        <div class="factura-item-info">
          <div class="factura-item-nombre">${item.nombre}</div>
          <div class="factura-item-cantidad">Cantidad: ${item.cantidad} x $${item.precioUnitario.toFixed(2)} USD</div>
        </div>
        <div class="factura-item-precio">$${item.total.toFixed(2)} USD</div>
      </div>
    `,
      )
      .join("")
    ...
    // Actualizar totales
    document.getElementById("factura-cantidad-total").textContent = CartManager.getTotalItems()
    document.getElementById("factura-total").textContent = `$${CartManager.getTotalPrice().toFixed(2)} USD`

    // Actualizar datos del comprador
    document.getElementById("factura-nombre").textContent = formData.nombreApellido
    document.getElementById("factura-dni").textContent = formData.dni
    document.getElementById("factura-email").textContent = formData.gmail
    document.getElementById("factura-telefono").textContent = formData.telefono
    document.getElementById("factura-metodo-entrega").textContent =
      formData.metodoEntrega === "retiro" ? "Retiro en Persona" : "Envío por Vía Cargo"

    // Mostrar información según método de entrega
    if (formData.metodoEntrega === "retiro") {
      document.getElementById("factura-info-retiro").style.display = "block"
      document.getElementById("factura-info-domicilio").style.display = "none"
      document.getElementById("factura-fecha-retiro").textContent =
        `${formData.diaRetiro} a las ${formData.horarioRetiro}`

      // Efectivo disponible solo para retiro
      efectivoCard.style.opacity = "1"
      efectivoCard.style.pointerEvents = "auto"
      document.querySelector('input[name="metodo-pago"][value="efectivo"]').disabled = false
    } else {
      document.getElementById("factura-info-retiro").style.display = "none"
      document.getElementById("factura-info-domicilio").style.display = "block"
      document.getElementById("factura-direccion").textContent =
        `${formData.direccion} ${formData.altura}${formData.pisoDepto ? ", " + formData.pisoDepto : ""}`
      document.getElementById("factura-localidad").textContent = formData.localidad
      document.getElementById("factura-provincia").textContent = formData.provincia
      document.getElementById("factura-codigo-postal").textContent = formData.codigoPostal

      // Efectivo NO disponible para envío
      efectivoCard.style.opacity = "0.5"
      efectivoCard.style.pointerEvents = "none"
      document.querySelector('input[name="metodo-pago"][value="efectivo"]').disabled = true
      document.querySelector('input[name="metodo-pago"][value="mercadopago"]').checked = true

      // Actualizar botones
      document.getElementById("realizar-pedido-btn").style.display = "none"
      document.getElementById("pagar-mercadopago-btn").style.display = "flex"
      document.getElementById("efectivo-disclaimer").style.display = "none"
      document.getElementById("mercadopago-disclaimer").style.display = "block"
    }

    // Iniciar animación del canvas
    this.animateFacturaCanvas()
  },

  initFacturaCanvas() {
    this.facturaCanvas = document.getElementById("factura-canvas")
    if (!this.facturaCanvas) return

    this.facturaCtx = this.facturaCanvas.getContext("2d")
    this.facturaParticles = []
    this.facturaAnimationId = null

    this.resizeFacturaCanvas()
    this.createFacturaParticles()
  ...
  resizeFacturaCanvas() {
    if (!this.facturaCanvas) return
    this.facturaCanvas.width = window.innerWidth
    this.facturaCanvas.height = window.innerHeight
  },

  createFacturaParticles() {
    this.facturaParticles = []
    const particleCount = Math.floor((this.facturaCanvas.width * this.facturaCanvas.height) / 20000)

    for (let i = 0; i < particleCount; i++) {
      this.facturaParticles.push({
        x: Math.random() * this.facturaCanvas.width,
        y: Math.random() * this.facturaCanvas.height,
        size: Math.random() * 3 + 1,
        speedX: (Math.random() - 0.5) * 0.3,
        speedY: (Math.random() - 0.5) * 0.3,
        opacity: Math.random() * 0.4 + 0.2,
  ...
  animateFacturaCanvas() {
    if (!this.facturaCtx) return

    const isDark = document.body.classList.contains("dark")
    this.facturaCtx.clearRect(0, 0, this.facturaCanvas.width, this.facturaCanvas.height)

    this.facturaParticles.forEach((particle, index) => {
      particle.x += particle.speedX
      particle.y += particle.speedY

      if (particle.x < 0 || particle.x > this.facturaCanvas.width) {
        particle.speedX *= -1
      }
      if (particle.y < 0 || particle.y > this.facturaCanvas.height) {
        particle.speedY *= -1
      }
      ...
      this.facturaCtx.beginPath()
      this.facturaCtx.arc(particle.x, particle.y, particle.size * pulseFactor, 0, Math.PI * 2)
      this.facturaCtx.fillStyle = baseColor
      this.facturaCtx.fill()

      for (let j = index + 1; j < this.facturaParticles.length; j++) {
        const other = this.facturaParticles[j]
        const dx = particle.x - other.x
        const dy = particle.y - other.y
        const distance = Math.sqrt(dx * dx + dy * dy)
        ...
        if (distance < 100) {
          ...
          this.facturaCtx.beginPath()
          this.facturaCtx.strokeStyle = lineColor
          this.facturaCtx.lineWidth = 0.5
          this.facturaCtx.moveTo(particle.x, particle.y)
          this.facturaCtx.lineTo(other.x, other.y)
          this.facturaCtx.stroke()
        }
      }
    })

    this.facturaAnimationId = requestAnimationFrame(() => this.animateFacturaCanvas())
  },
}

const PagoExitosoManager = {
  init() {
  ...
  setupEventListeners() {
    ...
    if (volverInicioBtn) {
      volverInicioBtn.addEventListener("click", () => {
        document.getElementById("pago-exitoso-seccion").style.display = "none"
        document.body.style.overflow = ""
        window.scrollTo({ top: 0, behavior: "smooth" })
      })
    }
  ...
  checkPaymentStatus() {
    ...
    if (paymentId && status === "approved") {
      this.mostrarPagoExitoso(paymentId, externalReference)
    }
  },

  async mostrarPagoExitoso(paymentId, externalReference) {
    try {
      ...
      if (response.ok && data.success) {
        document.getElementById("pago-exitoso-seccion").style.display = "flex"
        document.getElementById("numero-pedido").textContent = externalReference || paymentId
        document.getElementById("total-pagado").textContent = `$${data.amount} USD`
        document.body.style.overflow = "hidden"
